...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None, key_padding_mask=None):
        """
            query: [tgt_len, batch_size, hidden_size], tgt_len 表示目标序列长度
            key:   [src_len, batch_size, hidden_size], src_len 表示源序列长度
            value: [src_len, batch_size, hidden_size], src_len 表示源序列长度
            attn_mask: [tgt_len, src_len] or [num_heads * batch_size, tgt_len, src_len]
            在BERT中, attention_mask指代的其实是key_padding_mask. Defaults to None.
            key_padding_mask: [batch_size, src_len]. Defaults to None.
        Returns:
...
class BertModel(nn.Module):
    ...
    def forward(self,
        ...
        # [num_layers, src_len, batch_size, hidden_size]
        ...
        # 默认是最后一层的first token即 [cls] 位置经过 dense + tanh 后的结果
