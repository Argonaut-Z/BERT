...
class BertForNextSentencePrediction(nn.Module):
    ...
    def forward(self,
                ...
                token_type_ids=None,    # [src_len, batch_size] 如果输入模型的只有一个序列，那么这个参数不需要传值
                position_ids=None,
        ...
        # seq_relationship_score.shape = [batch_size, 2]
...
class BertForLMTransformHead(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        return:
            经过线性变换、激活函数、归一化以及最终映射到词汇表空间后的张量
            形状为[src_len, batch_size, vocab_size]
        """
...
class BertForMaskedLm(nn.Module):
    ...
    def forward(self,
        ...
        # sequence_output.shape = [src_len, batch_size, hidden_size]
        predicition_scores = self.classifier(sequence_output)
        # prediction_scores.shape = [src_len, batch_size, vocab_size]
        if masked_lm_labels is not None:
            loss_fct = nn.CrossEntropyLoss(ignore_index=0) # 忽略标签为0的项
            masked_lm_loss = loss_fct(predicition_scores.reshape(-1, self.config.vocab_size),   # [src_len  * batch_size, vocab_size]
                                        masked_lm_labels.reshape(-1))   # [src_len * batch_size,]
            return masked_lm_loss
        else:
            return predicition_scores   # [src_len, batch_size, vocab_size]
...
class BertForPretrainingModel(nn.Module):
    ...
    def __init__(self, config, bert_pretrained_model_dir=None):
        ...
        if 'use_embedding_weight' in config.__dict__ and config.use_embedding_weight:
            weights = self.bert.bert_embeddings.word_embeddings.embedding.weight
            logging.info(f"## 使用token embedding中的权重矩阵作为输出层的权重！{weights.shape}")
        self.mlm_predicition = BertForLMTransformHead(config, weights)
    ...
    def forward(self,
        ...
        pooled_output, all_encoder_outputs = self.bert(
            ...
            position_ids=position_ids)
        sequence_output = all_encoder_outputs[-1]   # 取BERT最后一层的输出
        # sequnce_output.shape = [src_len, batch_size, hidden_size]
        mlm_prediction_logits = self.mlm_predicition(sequence_output)
        # mlm_prediction_logits.shape = [src_len, batch_size, vocab_size]
        nsp_pred_logits = self.nsp_prediction(pooled_output)
        # nsp_pred_logits.shape = [batch_size, 2]
        if masked_lm_labels is not None and next_sentence_labels is not None:
            loss_fct_mlm = nn.CrossEntropyLoss(ignore_index=0)
            # MLM任务在构造数据集时padding部分和MASK部分都是用的0来填充，所以ignore_indesx需要指定为0
